  ...
  return (
    <div ref={ref} className={`scroll-reveal ${isVisible ? "visible" : ""} grid md:grid-cols-2 gap-8 md:gap-12 items-center`}>
        ...
        <div className="relative aspect-[4/3] rounded-2xl overflow-hidden shadow-xl group">
          ...
          {property.status && (
            <div className="absolute top-4 left-4">
              <Badge className={`font-body text-xs px-3 py-1 ${property.status === "Available" ? "bg-green-600 text-white" : property.status === "For Rent" ? "bg-blue-600 text-white" : "bg-muted text-muted-foreground"}`}>
                {property.status}
              </Badge>
            </div>
          )}
      ...
      <div className={`${isEven ? "" : "md:order-1"}`}>
        <h2 className="font-display text-3xl md:text-4xl font-semibold text-foreground mb-4">{property.title}</h2>
        {property.location && (
